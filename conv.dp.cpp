...
#define FETCH_FLOAT4(pointer) (reinterpret_cast<sycl::float4 *>(&(pointer))[0])
...
void v1_convolution(element_type *in, element_type *out, element_type *kernel, int batch_size,
                               int inC, int inH, int inW,
                               int outC, int outH, int outW,
                               int kernelH, int kernelW,
                               const sycl::nd_item<3> &item_ct1,
                               sycl::local_accessor<float, 2> s_kernel,
                               sycl::local_accessor<float, 2> s_in)
{
    // block id 与 thread id的读取与计算 分块是对target矩阵去分的
    // 目前按一个线程负责一个in的计算
    int block_row = item_ct1.get_group(1);
    int block_col = item_ct1.get_group(2);
    int thread_row = item_ct1.get_local_id(1),
        thread_col = item_ct1.get_local_id(2);
    ...
    /// 转移存储 GMEM --> SMEM
    // __shared__ float s_in[BLOCK_HEIGHT + KERNEL_HEIGHT - 1][BLOCK_WIDTH +
    // KERNEL_WIDTH - 1]; 开奇数内存会出错 要满足修正的尺寸
    float load_reg[4];
...
void v2_convolution(element_type *org,
                               element_type *target,
                               element_type *filter,
                               int row, int col,
                               const sycl::nd_item<3> &item_ct1,
                               const sycl::stream &stream_ct1,
                               sycl::local_accessor<float, 2> s_filter,
                               sycl::local_accessor<float, 2> s_org)
{
    // block id 与 thread id的读取与计算 分块是对target矩阵去分的
    int block_row = item_ct1.get_group(1);
    int block_col = item_ct1.get_group(2);
    int thread_row = item_ct1.get_local_id(1),
        thread_col = item_ct1.get_local_id(2),
        tid = thread_row * THREAD_SIZE_ROW + thread_col;
    // 目标矩阵尺寸
